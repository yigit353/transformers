...
class InputFeatures:
    ...
    word_position_ids: Optional[List[int]] = None
...
class TokenClassificationTask:
    ...
    def convert_examples_to_features(
        ...
        for ex_index, example in enumerate(examples):
            ...
            word_position_ids = tokenizer.create_word_ids_from_sequence(input_ids)
            subword_ids = tokenizer.create_subword_ids_from_sequence(input_ids)

            assert len(input_ids) == max_seq_length
            assert len(input_mask) == max_seq_length
            assert len(segment_ids) == max_seq_length
            assert len(word_position_ids) == max_seq_length
            assert len(subword_ids) == max_seq_length
            ...
            if ex_index < 5:
                ...
                logger.info("word_position_ids: %s", " ".join([str(x) for x in word_position_ids]))
                logger.info("subword_ids: %s", " ".join([str(x) for x in subword_ids]))
            ...
            features.append(
                InputFeatures(
                    ...
                    word_position_ids=word_position_ids, subword_ids=subword_ids, label_ids=label_ids,
                )
            )
        return features
