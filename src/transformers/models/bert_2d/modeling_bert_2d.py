...
BERT_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `({0})`):
            ...
            Indices can be obtained using [`Bert2dTokenizer`]. See [`PreTrainedTokenizer.encode`] and
...
class Bert2dForPreTraining(Bert2dPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(BERT_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @replace_return_docstrings(output_type=Bert2dForPreTrainingOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
            self,
            input_ids: Optional[torch.Tensor] = None,
            attention_mask: Optional[torch.Tensor] = None,
        ...
        ```python
        >>> from transformers import Bert2dTokenizer, Bert2dForPreTraining
        >>> import torch

        >>> tokenizer = Bert2dTokenizer.from_pretrained("bert-base-uncased")
        >>> model = Bert2dForPreTraining.from_pretrained("bert-base-uncased")
...
class Bert2dLMHeadModel(Bert2dPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = Bert2dModel(config, add_pooling_layer=False)
...
class Bert2dForNextSentencePrediction(Bert2dPreTrainedModel):
    ...
    def forward(
        ...
        ```python
        >>> from transformers import Bert2dTokenizer, Bert2dForNextSentencePrediction
        >>> import torch

        >>> tokenizer = Bert2dTokenizer.from_pretrained("bert-base-uncased")
        >>> model = Bert2dForNextSentencePrediction.from_pretrained("bert-base-uncased")
