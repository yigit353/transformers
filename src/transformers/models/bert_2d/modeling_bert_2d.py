# coding=utf-8
# Copyright 2022 Yiğit Bekir Kaya The HuggingFace Inc. team. All rights reserved.
#
...
def load_tf_weights_in_bert_2d(model, config, tf_checkpoint_path):
...
class Bert2dAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = Bert2dSelfAttention(config, position_embedding_type=position_embedding_type)
...
class Bert2dPreTrainedModel(PreTrainedModel):
    ...
    load_tf_weights = load_tf_weights_in_bert_2d
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, Bert2dEncoder):
...
BERT_2D_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `({0})`):
            ...
            Indices can be obtained using [`Bert2dTokenizer`].
...
@add_start_docstrings(
    "The bare Bert2D Model transformer outputting raw hidden-states without any specific head on top.",
...
class Bert2dForMaskedLM(Bert2dPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert_2d = Bert2dModel(config)
...
class Bert2dForCausalLM(Bert2dPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `Bert2dForCausalLM` as a standalone, add `is_decoder=True.`")
    ...
    def forward(
        ...
        ```python
        >>> from transformers import Bert2dTokenizer, Bert2dForCausalLM, Bert2dConfig
        >>> import torch

        >>> tokenizer = Bert2dTokenizer.from_pretrained('bert-2d-base-uncased')
        >>> config = Bert2dConfig.from_pretrained("bert-2d-base-uncased")
        >>> config.is_decoder = True
        >>> model = Bert2dForCausalLM.from_pretrained('bert-2d-base-uncased', config=config)
...
class Bert2dClassificationHead(nn.Module):
...
@add_start_docstrings(
    """Bert2D Model transformer with a sequence classification/regression head on top (a linear layer on top of
...
class Bert2dForSequenceClassification(Bert2dPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert_2d = Bert2dModel(config)
        self.classifier = Bert2dClassificationHead(config)
