# coding=utf-8
# Copyright 2022 Yiğit Bekir Kaya and The HuggingFace Inc. team. All rights reserved.
#
...
_CHECKPOINT_FOR_DOC = "bert-2d-base-uncased"
_CONFIG_FOR_DOC = "Bert2dConfig"
_TOKENIZER_FOR_DOC = "Bert2dTokenizer"

TF_BERT_2D_PRETRAINED_MODEL_ARCHIVE_LIST = [
    "bert-2d-base-uncased",
    # See all Bert2D models at https://huggingface.co/models?filter=bert_2d
]


class TFBert2dEmbeddings(tf.keras.layers.Layer):
    """Construct the embeddings from word, position and token_type embeddings."""

    def __init__(self, config: Bert2dConfig, **kwargs):
...
class TFBert2dSelfAttention(tf.keras.layers.Layer):
    def __init__(self, config: Bert2dConfig, **kwargs):
        super().__init__(**kwargs)
    ...
    def call(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in TFBert2dModel call() function)
...
class TFBert2dSelfOutput(tf.keras.layers.Layer):
    def __init__(self, config: Bert2dConfig, **kwargs):
        super().__init__(**kwargs)
...
class TFBert2dAttention(tf.keras.layers.Layer):
    def __init__(self, config: Bert2dConfig, **kwargs):
        super().__init__(**kwargs)

        self.self_attention = TFBert2dSelfAttention(config, name="self")
        self.dense_output = TFBert2dSelfOutput(config, name="output")
...
class TFBert2dIntermediate(tf.keras.layers.Layer):
    def __init__(self, config: Bert2dConfig, **kwargs):
        super().__init__(**kwargs)
...
class TFBert2dOutput(tf.keras.layers.Layer):
    def __init__(self, config: Bert2dConfig, **kwargs):
        super().__init__(**kwargs)
...
class TFBert2dLayer(tf.keras.layers.Layer):
    def __init__(self, config: Bert2dConfig, **kwargs):
        super().__init__(**kwargs)

        self.attention = TFBert2dAttention(config, name="attention")
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = TFBert2dAttention(config, name="crossattention")
        self.intermediate = TFBert2dIntermediate(config, name="intermediate")
        self.bert_output = TFBert2dOutput(config, name="output")
...
class TFBert2dEncoder(tf.keras.layers.Layer):
    def __init__(self, config: Bert2dConfig, **kwargs):
        super().__init__(**kwargs)
        self.config = config
        self.layer = [TFBert2dLayer(config, name=f"layer_._{i}") for i in range(config.num_hidden_layers)]
...
class TFBert2dPredictionHeadTransform(tf.keras.layers.Layer):
    def __init__(self, config: Bert2dConfig, **kwargs):
        super().__init__(**kwargs)
...
class TFBert2dLMPredictionHead(tf.keras.layers.Layer):
    def __init__(self, config: Bert2dConfig, input_embeddings: tf.keras.layers.Layer, **kwargs):
        super().__init__(**kwargs)

        self.vocab_size = config.vocab_size
        self.hidden_size = config.hidden_size

        self.transform = TFBert2dPredictionHeadTransform(config, name="transform")
...
class TFBert2dMLMHead(tf.keras.layers.Layer):
    def __init__(self, config: Bert2dConfig, input_embeddings: tf.keras.layers.Layer, **kwargs):
        super().__init__(**kwargs)

        self.predictions = TFBert2dLMPredictionHead(config, input_embeddings, name="predictions")
...
@keras_serializable
class TFBert2dMainLayer(tf.keras.layers.Layer):
    config_class = Bert2dConfig

    def __init__(self, config: Bert2dConfig, add_pooling_layer: bool = True, **kwargs):
        super().__init__(**kwargs)

        self.config = config
        self.is_decoder = config.is_decoder

        self.embeddings = TFBert2dEmbeddings(config, name="embeddings")
        self.encoder = TFBert2dEncoder(config, name="encoder")
...
class TFBert2dPreTrainedModel(TFPreTrainedModel):
...
BERT_2D_START_DOCSTRING = r"""
    ...
    Args:
        config ([`~Bert2dConfig`]): Model configuration class with all the parameters of the model.
            Initializing with a config file does not load the weights associated with the model, only the configuration.
            Check out the [`~PreTrainedModel.from_pretrained`] method to load the model weights.
"""

BERT_2D_INPUTS_DOCSTRING = r"""
    Args:
...
@add_start_docstrings(
    "The bare Bert2D Model transformer outputing raw hidden-states without any specific head on top.",
    BERT_2D_START_DOCSTRING,
)
class TFBert2dModel(TFBert2dPreTrainedModel):
    def __init__(self, config: Bert2dConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)

        self.bert_2d = TFBert2dMainLayer(config, name="bert_2d")

    @unpack_inputs
    @add_start_docstrings_to_model_forward(BERT_2D_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @add_code_sample_docstrings(
...
@add_start_docstrings("""Bert2D Model with a `language modeling` head on top. """, BERT_2D_START_DOCSTRING)
class TFBert2dForMaskedLM(TFBert2dPreTrainedModel, TFMaskedLanguageModelingLoss):
    ...
    def __init__(self, config: Bert2dConfig, *inputs, **kwargs):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `TFBert2dForMaskedLM` make sure `config.is_decoder=False` for "
                "bi-directional self-attention."
            )

        self.bert_2d = TFBert2dMainLayer(config, name="bert_2d")
        self.mlm = TFBert2dMLMHead(config, input_embeddings=self.bert_2d.embeddings, name="mlm___cls")
...
@add_start_docstrings(
    """Bert2D Model with a `language modeling` head on top for CLM fine-tuning. """, BERT_2D_START_DOCSTRING
)
class TFBert2dForCausalLM(TFBert2dPreTrainedModel, TFCausalLanguageModelingLoss):

    def __init__(self, config: Bert2dConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)

        if not config.is_decoder:
            logger.warning("If you want to use `TFBert2dForCausalLM` as a standalone, add `is_decoder=True.`")
...
class TFBert2dClassificationHead(tf.keras.layers.Layer):
...
@add_start_docstrings(
    """Bert2D Model transformer with a sequence classification/regression head on top
    e.g., for GLUE tasks. """,
    BERT_2D_START_DOCSTRING,
)
class TFBert2dForSequenceClassification(TFBert2dPreTrainedModel, TFSequenceClassificationLoss):
    def __init__(self, config: Bert2dConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        ...
        self.bert_2d = TFBert2dMainLayer(config, name="bert_2d")
        self.classifier = TFBert2dClassificationHead(config, name="classifier")
...
@add_start_docstrings(
    """Bert2D Model with a multiple choice classification head on top (a linear layer on top of
    the pooled output and a softmax) e.g. for RocStories/SWAG tasks. """,
    BERT_2D_START_DOCSTRING,
)
class TFBert2dForMultipleChoice(TFBert2dPreTrainedModel, TFMultipleChoiceLoss):
    def __init__(self, config: Bert2dConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
    ...
    @unpack_inputs
    @add_start_docstrings_to_model_forward(BERT_2D_INPUTS_DOCSTRING.format("batch_size, num_choices, sequence_length"))
    @add_code_sample_docstrings(
        processor_class=_TOKENIZER_FOR_DOC,
        checkpoint=_CHECKPOINT_FOR_DOC,
...
@add_start_docstrings(
    """Bert2D Model with a token classification head on top (a linear layer on top of
    the hidden-states output) e.g. for Named-Entity-Recognition (NER) tasks. """,
    BERT_2D_START_DOCSTRING,
)
class TFBert2dForTokenClassification(TFBert2dPreTrainedModel, TFTokenClassificationLoss):
...
class TFBert2dForQuestionAnswering(TFBert2dPreTrainedModel, TFQuestionAnsweringLoss):
